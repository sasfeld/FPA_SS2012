 ...
 * 
 */
public class SubjectFilter extends MessageFilter {

  /**
   * FIlter for the subjects
   * 
   * @param filterText
   *          - the text to filter
   * @param filterOp
   ...
   */
  public SubjectFilter(String filterText, FilterOperator filterOp) {
  ...
  public Set<Message> filter(Iterable<Message> messagesToFilter) {
    ...
    for (Message msg : messagesToFilter) {
      if (StringCompareHelper.matches(msg.getSubject(), this.filterText, this.operator)) {
        results.add(msg);
      }
    }
    return results;
