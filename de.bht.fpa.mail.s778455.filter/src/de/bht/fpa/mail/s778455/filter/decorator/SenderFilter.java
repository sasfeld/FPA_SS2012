 ...
 * 
 */
public class SenderFilter extends MessageFilter {
   ...
   */
  public SenderFilter(String filterText, FilterOperator filterOp) {
  ...
  public Set<Message> filter(Iterable<Message> messagesToFilter) {
    ...
    for (Message msg : messagesToFilter) {
      if (StringCompareHelper.matches(msg.getSender().getEmail(), this.filterText, this.operator)
          || StringCompareHelper.matches(msg.getSender().getPersonal(), this.filterText, this.operator)) {
        results.add(msg);
      }
    }
    return results;
